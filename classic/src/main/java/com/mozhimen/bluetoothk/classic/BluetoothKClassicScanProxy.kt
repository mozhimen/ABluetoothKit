...
open class BluetoothKClassicScanProxy : BaseWakeBefDestroyLifecycleObserver(), IBluetoothKScanProxy {
    ...
    override fun startScan(activity: Activity) {
        UtilBluetooth.requestBluetoothPermission(activity, onGranted = {
            ...
            if (!BluetoothKClassic.instance.isBluetoothEnabled())
                BluetoothKClassic.instance.getBluetoothAdapter()?.enable()
            cancelScan()
            BluetoothKClassic.instance.getBluetoothAdapter()?.startDiscovery()
        })
    }

    @OptIn(OApiInit_InApplication::class)
    override fun isScanning(): Boolean =
        BluetoothKClassic.instance.getBluetoothAdapter()?.isDiscovering == true

    @OptIn(OApiInit_InApplication::class)
    override fun cancelScan() {
        if (isScanning()) {
            BluetoothKClassic.instance.getBluetoothAdapter()?.cancelDiscovery()
        }
