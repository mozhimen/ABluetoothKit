...
class BluetoothKBleServerProxy : BaseWakeBefDestroyLifecycleObserver(), IBluetoothKProxy {
        ...
        BluetoothKUtil.requestBluetoothPermission(activity) {
            if (BluetoothKBle.instance.getBluetoothLeAdvertiser() != null) {
                ...
                val scanResponse = AdvertiseData.Builder()
                    ...
                    .addServiceUuid(ParcelUuid(CBluetoothKBle.UUID_SERVICE.str2uUID())) //服务UUID
                ...
                val bluetoothGattService = BluetoothGattService(CBluetoothKBle.UUID_SERVICE.str2uUID(), BluetoothGattService.SERVICE_TYPE_PRIMARY)

                //添加可读+通知characteristic
                bluetoothGattService.addCharacteristic(
                    BluetoothGattCharacteristic(CBluetoothKBle.UUID_CHARACTERISTIC_READ.str2uUID(), BluetoothGattCharacteristic.PROPERTY_READ or BluetoothGattCharacteristic.PROPERTY_NOTIFY, BluetoothGattCharacteristic.PERMISSION_READ).apply {
                        addDescriptor(BluetoothGattDescriptor(CBluetoothKBle.UUID_DESCRIPTOR.str2uUID(), BluetoothGattCharacteristic.PERMISSION_WRITE))
                    }
                )

                //添加可写characteristic
                bluetoothGattService.addCharacteristic(
                    BluetoothGattCharacteristic(CBluetoothKBle.UUID_CHARACTERISTIC_WRITE.str2uUID(), BluetoothGattCharacteristic.PROPERTY_WRITE, BluetoothGattCharacteristic.PERMISSION_WRITE)
                )
